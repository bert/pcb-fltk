/*!
 * \file src/hid/fltk/modal_dialog.cpp
 *
...
Modal_Dialog::Modal_Dialog (Fl_Window *top, const char *t, Icon c, bool cancel) :
  _icon_type (c),
  _title (t),
  _subject (),
  _message (),
...
void Modal_Dialog::refresh ()
{
  ...
  int w = std::max (std::max (bw, hw) + bwd + OS_FONT_SIZE, _min_w);
  int h = 10;
  int ww = w - 20;
  int heading_h = 25;
  int btn_w = 80;
